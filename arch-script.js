(function(){
var root=document.getElementById('arch-root');
// ── 場域（背景色塊，非框）──
var ZONES=[
...
var NODES=[
  ...
  /* ── 現場指揮層：班長制核心 ── */
  {id:'teamSession', nm:'動態編組', ico:'🪖', x:60, y:18, status:'planned', kind:'core',
    script:'TeamSession 取代固定 Squad 的核心差異：每次出班臨時組隊，同一批志工在不同梯次可以屬於不同編組。這比固定班組更貼近慈濟實際出班方式——人員流動性高，不同任務需要不同技能組合。TeamSession 有明確的生命週期（建立→集合→執行→收班），結束後解散，不留固定結構。指揮鏈（總指揮→帶隊幹部→組員）在 session 內有效，跨 session 由 Tier 串接。',
    desc:'任務導向動態編組。每梯次獨立 session，綁定 Tier/Location/Resource。組長手機即時看到本組人數與狀態。生命週期：建立→集合→執行→收班→解散。'},
  ...
  {id:'aidRecord', nm:'AidRecord 援助紀錄', ico:'💰', x:76, y:46, status:'planned', kind:'core',
    script:'AidRecord 是多型態的援助紀錄，取代原本只涵蓋現金的「金援帳本」。四種型態共用同一簽收鏈：金援（現金發放）、物資（大型設備/生活包）、修繕（工程委託）、重建（長期追蹤）。每筆 AidRecord 綁定 Case（個案）作為 Aggregate Root，Case 生命週期比單筆援助更長——一個案家可能歷經物資→修繕→重建三個階段，但只有一份 Case。不可刪帳本、雙人見證、受災戶簽名三個控制點在所有型態中一致執行。',
    desc:'多型態援助：金援/物資/修繕/重建，皆綁定 Case。身份確認→雙人見證→受災戶簽名→存證照→不可刪帳本。Gmail 推財務，月報 CSV 匯出。'},
  ...
  {id:'resource', nm:'Resource 資源', ico:'🚛', x:90, y:18, status:'planned', kind:'core',
    script:'Resource 取代原本只管「車輛」的 Vehicle 概念，改為更廣義的可調度資源：車輛（大卡車/廂型車/機車）、設備（抽水機/發電機/高壓清洗）、場地（收容空間/倉庫）。每個 Resource 有狀態（可用/派遣中/維修/回程）、位置（即時 GPS）、容量（載人/載物）和技能需求（需持照駕駛/操作證）。Resource 和 TeamSession 是多對多關係：一個 session 可調用多項資源，一項資源在不同梯次間轉調。',
    desc:'廣義可調度資源：車輛/設備/場地。狀態：可用→派遣中→回程。綁定 TeamSession，多對多關係。即時 GPS 追蹤。'},

  {id:'location', nm:'Location 地點', ico:'📍', x:85, y:34, status:'planned', kind:'core',
    script:'Location 獨立成 GIS Entity，是整個系統地理資訊的單一來源。過去 GPS 座標散落在各個模組（個案地址、任務地點、集合點、倉庫位置），造成重複輸入和資料不一致。Location 統一管理地點資料：地址、GPS 座標、地點類型（個案現場/集合點/物資站/醫療點）、行政區域、以及實時狀態（可進入/積水/封路）。TaskPool、TeamSession、Incident 和 AidRecord 都引用 Location ID，不重複儲存座標。',
    desc:'GIS 地點實體。欄位：address/GPS/type/district/accessStatus。被 TaskPool/TeamSession/Incident/AidRecord 引用。地點狀態（積水/封路）即時更新影響任務媒合。'}
];
...
var EDGES=[
  ...
  /* ── 人力鏈（Person→TeamSession→Task） ── */
  {from:'reg', to:'teamSession', label:'編入組別',
    talk:'志工報到後不只是「進名冊」，而是立刻被編入本梯次的 TeamSession。這條線讓組長手機即時看到本組人數，任務執行人力也同步更新。',
    flow:'報到資料寫入後，依梯次查詢對應 TeamSession，session.members +1，組長端即時更新。'},

  {from:'teamSession', to:'taskpool', label:'接單', bi:true,
    talk:'組長從任務池接單（或被 HQ 指派），接單後任務狀態從 open→claimed，組員同步收到 LINE 推播。這是 Uber 模式的核心迴路。',
    flow:'雙向。去程(HQ→編組)：指派任務。回程(編組→任務池)：組長搶單，更新 task.claimedBy 和 claimedAt。'},

  {from:'tier', to:'teamSession', label:'建立編組',
    talk:'梯次確認時同步建立 TeamSession，把志工、資源、地點三者綁定。梯次是時間骨架，TeamSession 是執行單元，這條線讓「誰、去哪、帶什麼」在出班前就確定。',
    flow:'梯次建立→為每個任務群組產生 TeamSession，綁定成員/Resource/Location，推播給組長（LINE OA）。'},

  /* ── 後勤鏈（Resource→TeamSession） ── */
  {from:'tier', to:'resource', label:'資源配對', dash:true,
    talk:'梯次建立時，系統同時檢查每個編組是否有足夠資源（車輛/設備）。缺資源時自動建議從可用池配對，確認後推播給資源負責人。',
    flow:'梯次建立→檢查 Resource 可用清單→缺口警示+配對建議→確認後 resource.assignedSession 寫入。'},

  {from:'resource', to:'teamSession', label:'資源派遣',
    talk:'資源出發後，組長手機收到預計抵達時間。資源抵達後掃碼確認，物資裝載觸發倉儲扣庫存。',
    flow:'resource.status = en-route → 組長端推播。抵達後確認 → cargo 掃描 → 倉儲自動扣庫存。'},

  /* ── GIS 鏈（Location） ── */
  {from:'location', to:'taskpool', label:'地點資訊',
    talk:'任務池的每筆任務都引用 Location Entity，不再重複儲存 GPS 座標。地點狀態（積水/封路）改變時，相關任務的媒合優先級自動調整。',
    flow:'Task.locationId → Location Entity。地點 accessStatus 更新→ 任務媒合重算。'},

  {from:'location', to:'cmdmap', label:'GIS 圖層',
    talk:'作戰指揮圖的所有地理圖層都從 Location Entity 讀取，確保地圖上的點位和任務/個案資料一致，不會出現地圖標示和系統紀錄座標不同的情況。',
    flow:'cmdmap 讀取 Location[] → 渲染任務熱力圖/編組位置/封路狀態圖層。'},

  {from:'location', to:'incident', label:'事故地點',
    talk:'人員意外回報時直接引用 Location ID，不需要重新輸入座標。同一地點若多次發生意外，系統自動標記為高風險區域並警示指揮官。',
  ...
  {from:'handover', to:'teamSession', label:'下梯交接',
  ...
  {from:'aidRecord', to:'backend', label:'帳務存檔',
    talk:'每筆 AidRecord 寫入後端後，依型態自動 Gmail 通知財務組（金援/物資）或工程組（修繕/重建），月底自動匯出總表 CSV。帳本只可新增，不可刪除或修改。',
    flow:'AidRecord 確認 → 寫不可刪帳本 → Gmail 推對應組別（附 PDF）→ 月底自動 CSV 匯出供對帳。'},

  /* ── 人員意外鏈 ── */
  {from:'teamSession', to:'incident', label:'意外回報',
...
var VIEWS={
  decider:{
    ...
    stories:[
      ...
      {step:'動態編組指揮',on:['teamSession','taskpool','tier','cmdmap','resource','location'],
...
var HEALTH={
  ...
  teamSession:  {s:'todo', issue:'Phase 0-A 核心。TeamSession schema 尚未建立（取代固定 Squad）。生命週期管理、組長手機介面、梯次綁定邏輯待開發。所有後續功能的地基。'},
  ...
  aidRecord:    {s:'todo', issue:'Phase 3-A。多型態援助（金援/物資/修繕/重建）共用簽收鏈。canvas 簽名、不可刪帳本、依型態 Gmail 推對應組別待開發。Case 作為 Aggregate Root 需先確認 schema。'},
  resource:     {s:'todo', issue:'Phase 1-B。Resource schema（取代 Vehicle）：車輛/設備/場地統一管理。即時 GPS、容量、狀態機（可用→派遣→回程）待開發。'},
  location:     {s:'todo', issue:'Phase 1-C 附帶。Location GIS Entity：統一地點資料來源。需整合現有 taskpool/incident/fieldgroup 中散落的 GPS 欄位。accessStatus（積水/封路）影響媒合邏輯。'},
...
function buildNodes(){NODES.forEach(function(n){var el=document.createElement('div');el.className='node '+n.status+(n.center?' center':'')+(n.trigger?' trigger':'')+(n.mini?' mini':'');el.id='arch-node-'+n.id;el.style.left=n.x+'%';el.style.top=n.y+'%';el.innerHTML='<div class="hbadge" id="arch-hb-'+n.id+'"></div><div class="st '+n.status+'"></div><div class="ico">'+n.ico+'</div><div class="nm">'+n.nm+'</div><div class="kd">'+n.kind+'</div>';stage.appendChild(el);attachDrag(el,n);});}
function buildMergeFrames(){MERGE_FRAMES.forEach(function(mf,fi){var el=document.createElement('div');el.className='merge-frame';el.id='arch-mf-'+fi;el.innerHTML='<div class="mf-lbl">'+mf.lbl+'</div>';stage.appendChild(el);});}
function positionMergeFrames(){MERGE_FRAMES.forEach(function(mf,fi){var el=document.getElementById('arch-mf-'+fi);if(!el)return;var xs=[],ys=[];mf.nodes.forEach(function(id){var n=nodeById[id];if(n){xs.push(n.x);ys.push(n.y);}});if(!xs.length)return;var pad=7;var minx=Math.min.apply(null,xs)-pad,maxx=Math.max.apply(null,xs)+pad,miny=Math.min.apply(null,ys)-pad,maxy=Math.max.apply(null,ys)+pad;el.style.left=minx+'%';el.style.top=miny+'%';el.style.width=(maxx-minx)+'%';el.style.height=(maxy-miny)+'%';});}
...
function drawEdge(e,i){var a=nodeById[e.from],b=nodeById[e.to];var pp=edgePath(a,b);var path=document.createElementNS('http://www.w3.org/2000/svg','path');path.setAttribute('d',pp.d);path.setAttribute('fill','none');path.setAttribute('stroke','#C4C4CC');path.setAttribute('stroke-width','2');path.setAttribute('marker-end','url(#arr)');if(e.dash)path.setAttribute('stroke-dasharray','5,5');if(e.bi)path.setAttribute('marker-start','url(#arrR)');path.setAttribute('id','arch-edge-'+i);path.onclick=(function(idx){return function(){selectEdge(idx);};})(i);svg.appendChild(path);var lab=document.createElement('div');lab.className='elabel';lab.id='arch-elabel-'+i;lab.style.left=(pp.lx/VW*100)+'%';lab.style.top=(pp.ly/VH*100)+'%';lab.textContent=e.label;lab.onclick=(function(idx){return function(){selectEdge(idx);};})(i);stage.appendChild(lab);}
function refreshEdges(){EDGES.forEach(function(e,i){var a=nodeById[e.from],b=nodeById[e.to];var pp=edgePath(a,b);var path=document.getElementById('arch-edge-'+i);if(path)path.setAttribute('d',pp.d);var lab=document.getElementById('arch-elabel-'+i);if(lab){lab.style.left=(pp.lx/VW*100)+'%';lab.style.top=(pp.ly/VH*100)+'%';}});}
...
function selectNode(id){
  ...
  if(storyOn){
    ...
    detail.innerHTML='<div class="detail-head"><div class="dico">'+n.ico+'</div><div class="dt"><h3>'+n.nm+'</h3><div class="meta">'+n.id+' · 故事模式暫停</div></div></div><div class="detail-body">'+scr+'<div style="font-size:11px;color:var(--text3);margin-bottom:12px">'+n.desc+'</div>'
      +'<button onclick="storyResume()" style="border:none;background:var(--accent);color:#fff;padding:8px 16px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer">▶ 繼續故事</button>'
      +'</div>';
    return;
  }
  // Normal mode
  clearSel();var el=document.getElementById('arch-node-'+id);if(el)el.classList.add('sel');var n=nodeById[id];EDGES.forEach(function(e,i){if(e.from===id||e.to===id){var p=document.getElementById('arch-edge-'+i);if(p){p.classList.add('edge-active');p.setAttribute('marker-end','url(#arrH)');if(e.bi)p.setAttribute('marker-start','url(#arrRH)');}}});var subs=n.subs?'<div class="sub-mods">'+n.subs.map(function(s){return '<span class="sub-mod">'+s+'</span>';}).join('')+'</div>':'';var hext='';if(curView==='health'&&HEALTH[id]){var hs=HEALTH[id].s;var hcol={done:'green',todo:'amber',risk:'red',merge:'planned'}[hs];hext='<div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--border)"><span class="tag '+hcol+'">🩺 '+HEALTH_LBL[hs]+'</span> <span style="font-size:11px">'+HEALTH[id].issue+'</span></div>';}var scr=n.script?'<div style="background:linear-gradient(135deg,var(--accent-bg),var(--blue-bg));border:1px solid var(--accent-border);border-radius:var(--r-sm);padding:11px 13px;margin-bottom:10px;font-size:12.5px;line-height:1.85;color:var(--text)"><span style="font-size:10px;font-weight:700;color:var(--accent);letter-spacing:.05em">💡 設計說明</span><br>'+n.script+'</div>':'';var pvBtn='<div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border);display:flex;gap:8px;flex-wrap:wrap"><button onclick="arch_pvShow(\''+n.id+'\',\''+n.nm.replace(/'/g,'')+'\')" style="border:none;background:var(--accent);color:#fff;padding:8px 14px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer">🔍 看現況示意</button><button onclick="document.getElementById(\'arch-fbPanel\').classList.add(\'on\');if(!document.getElementById(\'arch-fbNode\').options.length)arch_fbBuildNodeOptions_inner();document.getElementById(\'arch-fbNode\').value=\''+n.nm.replace(/'/g,'')+'\'" style="border:1px solid var(--accent-border);background:var(--accent-bg);color:var(--accent);padding:8px 14px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer">📝 對此寫回饋</button></div>';detail.innerHTML='<div class="detail-head"><div class="dico">'+n.ico+'</div><div class="dt"><h3>'+n.nm+' <span class="tag '+n.status+'">'+statusTag[n.status]+'</span></h3><div class="meta">'+n.id+' · '+n.kind+'</div></div></div><div class="detail-body">'+scr+'<div style="font-size:11px;color:var(--text3)">'+n.desc+'</div>'+subs+hext+pvBtn+'</div>';}
// E7: resume story after node click pause
function storyResume(){ storyOn=true; document.getElementById('arch-storyBar').classList.add('show'); storyShow(); }
function selectEdge(i){if(storyOn)return;clearSel();var e=EDGES[i];var p=document.getElementById('arch-edge-'+i);if(p){p.classList.add('edge-active');p.setAttribute('marker-end','url(#arrH)');if(e.bi)p.setAttribute('marker-start','url(#arrRH)');}var lab=document.getElementById('arch-elabel-'+i);if(lab)lab.classList.add('sel');var a=nodeById[e.from],b=nodeById[e.to];var dir=e.bi?'\u2194 雙向':'\u2192 單向';var etalk=e.talk?'<div style="background:linear-gradient(135deg,var(--accent-bg),var(--blue-bg));border:1px solid var(--accent-border);border-radius:var(--r-sm);padding:11px 13px;margin-bottom:10px;font-size:12.5px;line-height:1.85;color:var(--text)"><span style="font-size:10px;font-weight:700;color:var(--accent);letter-spacing:.05em">💡 設計說明</span><br>'+e.talk+'</div>':'';detail.innerHTML='<div class="detail-head"><div class="dico">'+(e.bi?'\u2194':'\u2192')+'</div><div class="dt"><h3>'+a.nm+' '+(e.bi?'\u2194':'\u2192')+' '+b.nm+' <span class="tag '+(e.dash?'planned':'live')+'">'+(e.dash?'規劃/次要':'現行')+'</span></h3><div class="meta">'+dir+' · 「'+e.label+'」</div></div></div><div class="detail-body">'+etalk+'<div style="font-size:11px;color:var(--text3)">'+e.flow+'</div></div>';}

// ── 故事導覽 ──
var storyOn=false,storyIdx=0;
function storyStart(){storyOn=true;storyIdx=0;document.getElementById('arch-storyBar').classList.add('show');storyShow();}
function storyEnd(){storyOn=false;document.getElementById('arch-storyBar').classList.remove('show','warn-bar');clearDim();clearSel();applyView();detail.innerHTML='<div class="detail-empty">▲ 拖曳排版 · 點節點或線看說明 · 或按 ▶ 故事導覽</div>';}
...
function storyShow(){
  ...
  NODES.forEach(function(n){var el=document.getElementById('arch-node-'+n.id);if(el)el.classList.remove('h-done','h-todo','h-risk','h-merge');});
  var bar=document.getElementById('arch-storyBar');
  bar.classList.toggle('warn-bar',!!s.warn);
  document.getElementById('arch-storyStep').textContent=(storyIdx+1)+' / '+S.length+' · '+s.step;
  document.getElementById('arch-storyText').textContent=s.text;
  document.getElementById('arch-sPrev').disabled=(storyIdx===0);
  document.getElementById('arch-sNext').textContent=(storyIdx===S.length-1)?'✓':'›';
  ...
  NODES.forEach(function(n){var el=document.getElementById('arch-node-'+n.id);if(!el)return;el.classList.remove('hot','dim','warn');
    if(warnNodes[n.id])el.classList.add('warn');
    else if(onNodes[n.id])el.classList.add('hot');
    else el.classList.add('dim');});
  // 點亮兩端都在 on 集合內的線
  EDGES.forEach(function(e,i){var p=document.getElementById('arch-edge-'+i);var lab=document.getElementById('arch-elabel-'+i);
...
function clearHealth(){root.classList.remove('health-on');NODES.forEach(function(n){var el=document.getElementById('arch-node-'+n.id);if(el)el.classList.remove('h-done','h-todo','h-risk','h-merge');});}
function applyView(){
  clearDim();clearSel();clearHealth();
  if(curView==='health'){
    root.classList.add('health-on');
    positionMergeFrames();
    NODES.forEach(function(n){var el=document.getElementById('arch-node-'+n.id);if(!el)return;el.classList.remove('dim');var h=HEALTH[n.id];if(h){el.classList.add('h-'+h.s);var hb=document.getElementById('arch-hb-'+n.id);if(hb)hb.textContent=HEALTH_LBL[h.s];}});
    EDGES.forEach(function(e,i){var p=document.getElementById('arch-edge-'+i);var lab=document.getElementById('arch-elabel-'+i);if(p)p.classList.remove('dim');if(lab)lab.classList.remove('dim');});
    return;
  }
  if(curView==='all'){
    NODES.forEach(function(n){var el=document.getElementById('arch-node-'+n.id);if(el)el.classList.remove('dim');});
    EDGES.forEach(function(e,i){var p=document.getElementById('arch-edge-'+i);var lab=document.getElementById('arch-elabel-'+i);if(p)p.classList.remove('dim');if(lab)lab.classList.remove('dim');});
    return;
  }
  var set={};(VIEWS[curView].nodes||[]).forEach(function(id){set[id]=1;});
  NODES.forEach(function(n){var el=document.getElementById('arch-node-'+n.id);if(el)el.classList.toggle('dim',!set[n.id]);});
  EDGES.forEach(function(e,i){var on=set[e.from]&&set[e.to];var p=document.getElementById('arch-edge-'+i);var lab=document.getElementById('arch-elabel-'+i);if(p)p.classList.toggle('dim',!on);if(lab)lab.classList.toggle('dim',!on);});
}

function resetLayout(){NODES.forEach(function(n){n.x=DEFAULT_POS[n.id].x;n.y=DEFAULT_POS[n.id].y;var el=document.getElementById('arch-node-'+n.id);if(el){el.style.left=n.x+'%';el.style.top=n.y+'%';}});refreshEdges();savePos();}
...
function importLayout(){var s=window.prompt('貼上座標 JSON：','');if(!s)return;try{var p=JSON.parse(s);NODES.forEach(function(n){if(p[n.id]){n.x=p[n.id].x;n.y=p[n.id].y;var el=document.getElementById('arch-node-'+n.id);if(el){el.style.left=n.x+'%';el.style.top=n.y+'%';}}});refreshEdges();savePos();}catch(e){window.alert('JSON 格式錯誤');}}
...
function pvShow(id,name){
  var v=PV_VIEWS[id];
  document.getElementById('arch-pvTitle').textContent='現況示意　·　'+(name||(v&&v.title)||id);
  document.getElementById('arch-pvBody').innerHTML=v?v.render():pvPlaceholder(name||id);
  document.getElementById('arch-pvMask').classList.add('on');
}
function pvHide(){document.getElementById('arch-pvMask').classList.remove('on');}

/* ───── 回饋提交碼（相容 intake INTAKE1 格式） ───── */
function fbEncode(obj){return "INTAKE1:"+btoa(unescape(encodeURIComponent(JSON.stringify(obj))));}
var _fbCode="";
function arch_fbBuildNodeOptions_inner(){
  var sel=document.getElementById('arch-fbNode');if(!sel)return;
...
function fbT(){
  var p=document.getElementById('arch-fbPanel');p.classList.toggle('on');
  if(p.classList.contains('on')&&!document.getElementById('arch-fbNode').options.length)arch_fbBuildNodeOptions_inner();
}
function fbFlash(m,err){var e=document.getElementById('arch-fbFlash');e.textContent=m;e.style.color=err?'var(--red)':'var(--green)';setTimeout(function(){e.textContent='';},3500);}
function fbGenCode(){
  var nick=document.getElementById('arch-fbNick').value.trim();
  var text=document.getElementById('arch-fbText').value.trim();
  if(!text){fbFlash('請先填寫回饋內容',true);return;}
  var obj={kind:'feedback',nick:nick||'匿名',node:document.getElementById('arch-fbNode').value,
    ftype:document.getElementById('arch-fbType').value,text:text,ts:Date.now()};
  _fbCode=fbEncode(obj);
  document.getElementById('arch-fbCode').textContent=_fbCode;
  document.getElementById('arch-fbOut').classList.add('on');
...
window.arch_storyStart=storyStart;
window.arch_storyEnd=storyEnd;
window.arch_storyPrev=storyPrev;
window.arch_storyNext=storyNext;
window.arch_resetLayout=resetLayout;
window.arch_exportLayout=exportLayout;
window.arch_importLayout=importLayout;
window.arch_setView=setView;
window.arch_pvHide=pvHide;
window.arch_pvShow=pvShow;
window.arch_fbT=fbT;
