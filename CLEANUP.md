...
**建議理由：** commit 活躍度與 ARCH.md 的拆檔記錄都指向 index.html 是唯一在演進的主線，另兩者已無人更動超過 6–24 天。

**✅ 裁決結果（2026-07-04，Mason）：選 A。** Mason 證實「當時一個檔案太大，應該有拆」——`index_v4.0_20260622.html` 確為因檔案過大而拆分的母檔，`index.html` 為現行唯一主線。mason-claude-memory 的專案路由已同步更新。注意：本裁決只定「哪份是主檔」；下表廢檔候選的歸檔/刪除動作仍待 Mason 另行指示。
...
**✅ 裁決結果（2026-07-04，Mason，兩階段）：**
1. 第一階段：「不要隨便刪」——四檔全部保留，刪除選項作廢。
2. 第二階段（同日稍後）：Mason 核准 **tag/SHA 封存法**——四檔移出工作目錄（任何程序零接觸），本體由 git 歷史永久保留，錨點與一行還原指令見 `ARCHIVE.md`。這不是刪除：內容完整可隨時取回。
